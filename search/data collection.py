...
questions = pd.read_csv('/kaggle/input/aime-problem-set-1983-2024/AIME_Dataset_1983_2024.csv')
...
for _, row in questions.iterrows():
    ...
    cnt += 1
    if cnt % 10 == 0:
        res = pd.DataFrame(data, columns=['question', 'prompt_head', 'response', 'solved'])
        res.to_csv(f'data{cnt}.csv', index=False)
    random_thoughts = random.sample(thoughts, 5)
    results = predict_with_response(question, answer, random_thoughts)
    for result in results:  # Unpack each result into the data list
        data.append([result[0], result[1], result[2], result[4]])  # question, prompt_head, response, solved